...
{Fore.CYAN}3. NPM (Legacy){Fore.RESET}
        ...
        match pm:
            case 1:
                try:
                    subprocess.run(["npm", "install", "cnpm", "-g"], shell=False, text=True, check=True, stdout=subprocess.DEVNULL)
                except Exception:
                    print(f"{ERROR}Failed to install CNPM (安装CNPM失败)")
                    return False
                try:
                    subprocess.run(["cnpm", "install", "-g", "openclaw@latest"], shell=False, text=True, check=True)
                    if PlatformDetector.is_windows():
...
""")
            ...
            case 2:
                try:
                    subprocess.run(["npm", "install", "pnpm", "-g"], shell=False, text=True, check=True, stdout=subprocess.DEVNULL)
                    ...
                    self._refresh_env()
                    subprocess.run(["pnpm", "setup"], shell=False, text=True, check=True, stdout=subprocess.DEVNULL)
                except Exception:
                    print(f"{ERROR}Failed to install PNPM (安装PNPM失败)")
                    return False
                try:
                    subprocess.run(["pnpm", "add", "-g", "openclaw@latest"], shell=False, text=True, check=True)
                    if PlatformDetector.is_windows():
                        with open(os.path.join(os.environ["USERPROFILE"], "Desktop", "OpenClaw.cmd"), "w") as f:
                            f.write("@echo off & openclaw dashboard")
                    elif PlatformDetector.is_linux():
...
""")
            ...
            case 3:
                try:
                    subprocess.run(["npm", "install", "-g", "openclaw@latest"], shell=False, text=True, check=True)
                    if PlatformDetector.is_windows():
                        with open(os.path.join(os.environ["USERPROFILE"], "Desktop", "OpenClaw.cmd"), "w") as f:
                            f.write("@echo off & openclaw dashboard")
                    elif PlatformDetector.is_linux():
